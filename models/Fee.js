...
feeSchema.methods.generateInstallments = function() {
  ...
  this.installments = [
    ...
    {
      name: 'First Installment (25%)',
      amount: inst1Amount,
      dueDate: inst1Due,
      paidAmount: 0
    },
    {
      name: 'Second Installment (25%)',
      amount: inst2Amount,
      dueDate: inst2Due,
      paidAmount: 0
    }
...
feeSchema.methods.allocatePayments = function() {
  ...
  let remainingPaid = this.payments.reduce(
  ...
  const nextUnpaid = this.installments.find(
...
feeSchema.pre('save', function(next) {
  ...
  const isTotalModified =
    ...
    this.isModified('courseDurationMonths');

  const hasNoInstallments =
    !this.installments ||
    this.installments.length === 0;

  const isInstallmentsModified =
    this.isModified('installments');
...
feeSchema.virtual('paidPct').get(function() {
  ...
  return net > 0
    ? Math.round((this.paidAmount / net) * 100)
    : 0;
