...
async function convertImportedLead(lead, course, row, adminId) {
  ...
  const user = await User.create({
    ...
    role: 'student',
    defaultSimCode: String(lead.defaultSimCode || '').trim().toUpperCase().slice(0, 50),
    status: 'active',
...
async function createImportedStudent(course, row, adminId) {
  ...
  const user = await User.create({
    ...
    password: crypto.randomBytes(16).toString('hex'),
    role: 'student',
    defaultSimCode: String(row.sim_code || row.default_sim_code || row.sim || '').trim().toUpperCase().slice(0, 50),
    status: 'active',
    mustChangePassword: true,
    passwordSetByAdmin: true,
    firstLoginCompleted: false,
