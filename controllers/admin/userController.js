...
exports.getDashboard = async (req, res) => {
  try {
    const [totalStudents, totalTeachers, totalLeads, feeData, resetRequests, messages] = await Promise.all([
      User.countDocuments({ role: 'student', isActive: true }),
      User.countDocuments({ role: 'teacher', isActive: true }),
      Lead.countDocuments({ status: { $nin: ['converted', 'lost'] } }),
      Fee.find().select('totalAmount paidAmount discount'),
    ...
    const totalRevenue = feeData.reduce((s, f) => s + f.paidAmount, 0);
    const totalDue = feeData.reduce((s, f) => s + Math.max(0, f.totalAmount - f.discount - f.paidAmount), 0);
    ...
    res.render('admin/dashboard', {
      ...
      user: req.user,
      stats: { totalStudents, totalTeachers, totalLeads, totalRevenue, totalDue },
      recentStudents,
      hotLeads,
      resetRequests,
      messages,
...
exports.getCreateUser = (req, res) => {
  res.render('admin/user-form', { title: 'Add User', user: req.user, target: null, defaultRole: req.query.role || '' });
};
...
exports.postCreateUser = async (req, res) => {
  ...
  try {
    ...
    res.render('admin/user-form', { title: 'Add User', user: req.user, target: null, defaultRole: req.body.role || '', error: err.message });
  }
};
...
exports.getEditUser = async (req, res) => {
  try {
    ...
    res.render('admin/user-form', { title: 'Edit User', user: req.user, target, defaultRole: target.role });
  } catch (err) {
...
exports.postApproveProfileUpdate = async (req, res) => {
  try {
    ...
    res.redirect(`/admin/students/${student._id}?profile_approved=1`);
  } catch (err) {
...
exports.postRejectProfileUpdate = async (req, res) => {
  try {
    ...
    res.redirect(`/admin/students/${student._id}?profile_rejected=1`);
  } catch (err) {
