...
exports.updateProfile = async (req, res) => {
  ...
  try {
    if (req.user.role === 'student') {
      ...
      if (req.file) {
        updateRequest.profilePic = `/uploads/${req.file.filename}`;
      }
...
exports.getInbox = async (req, res) => {
  try {
    ...
    if (req.user.role === 'admin') {
      ...
      defaultContacts = await User.find({
        ...
        $or: [
          { role: 'admin' },
          { role: 'teacher' },
          { role: 'student', counsellor: req.user._id, isActive: true }
        ]
      })
        .select('name role profilePic rollNumber')
        .sort({ name: 1 });
      ...
      const ids = [];
      if (req.user.counsellor) ids.push(req.user.counsellor);
      if (req.user.teacher) ids.push(req.user.teacher);
    ...
    const contacts = Array.from(contactMap.values()).map(u => {
