...
exports.getAttendancePage = async (req, res) => {
  ...
  try {
    ...
    if (!subjects || subjects.length === 0) {
      subjects = ['Video Editing', 'Digital Marketing', 'Web Development', 'Python Basics'];
    }
...
exports.postMarkAttendance = async (req, res) => {
  ...
  try {
    if (req.user.role !== 'admin') {
      const validBatch = await Schedule.findOne({ teacher: req.user._id, batch });
      if (!validBatch) {
        console.warn(`⚠️ Teacher unauthorized attendance attempt: teacher ${req.user._id} attempted batch ${batch}`);
