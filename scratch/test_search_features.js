...
async function run() {
  ...
  try {
    ...
    // Search for student "Arjun"
    const searchTeacherRes = await makeRequest({
      hostname: 'localhost', port: 3125, path: '/teacher/students?search=Arjun', method: 'GET',
      headers: { 'Cookie': teacherCookie }
    });
    if (!searchTeacherRes.data.includes('Arjun Saxena')) {
      throw new Error('Teacher search failed: Arjun Saxena not found in search results!');
    }
